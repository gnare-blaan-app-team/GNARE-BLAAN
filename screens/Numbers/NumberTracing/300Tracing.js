        ...
        this.panResponder = PanResponder.create({
            ...
            onPanResponderMove: (e, gestureState)=> {
                ...
                if(gestureState.vx >= 1.2 || gestureState.vx <= -1.2) {
                    ...
                    // Dot 1
                    if((gestureState.moveX < (screenWidth / 2) - (screenWidth * 0.1)) && !this.state.dot8) {
                        if(!this.state.dot1) {
                            //alert('Dot1');
                        ...
                        if((gestureState.moveX < (screenWidth / 2) + (screenWidth * 0.1)) && this.state.dot8) {
                            // Dot 7
                            if(!this.state.dot9 && this.state.dot8) {
                        ...
                        // Trace Number Three, Zero (2)
                            ...
                            // Dot 7
                            if(!this.state.dot15 && this.state.dot14) {
                                if(e.nativeEvent.locationX >= this.line3[0].x - scope && e.nativeEvent.locationX <= this.line3[0].x + scope) {
                                    if(e.nativeEvent.locationY >= this.line3[0].y - scope && e.nativeEvent.locationY <= this.line3[0].y + scope) {
                                        //alert('Dot 7');
                        ...
                        if(this.state.dot14) {
                            if(this.state.dot20) {
                                ...
                                this.setState({arrayMove: [], tracing2: shadedLine[2], tracing3: tracingLine[2],
                                    ...
                                    dot19: false, dot20: false, dot15: false, });
                            }
                        } else {
                            this.setState({arrayMove: [], shaded: shadedLine[1], tracing2: tracingLine[2],
                                showTracing: 0, dot9: false, dot14: false, dot10: false,
                                dot13: false, dot11: false, dot12: false, showShaded: 1,});
                        }
                    } else {
                        this.setState({arrayMove: [], showShaded: 1, shaded: shadedLine[0],
                         tracing: tracingLine[1], dot5: false, dot8: false, dot7: false, dot6: false});
                    }
                } else {
                    this.clearBoard();
                }
